...
func checkAndCleanOrphanSecret(ctx context.Context, client *k8sclient.K8sClient, secrets *corev1.Secret) error {
	...
	// new version of dingofs-csi-driver has a label to identify the secret
	// no need to manual clean up
	if _, ok := secrets.Labels[config.DingofsSecretLabelKey]; ok {
		return nil
	}
	if !strings.HasPrefix(secrets.Name, "dingofs-") || !strings.HasSuffix(secrets.Name, "-secret") {
		return nil
	}
...
func (m *SecretController) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	dfsSetting, err := dfs.InitDfs(ctx, "", "", secretsMap, nil, nil)
	if err != nil {
		return reconcile.Result{}, err
	}
	tempConfDir, err := os.MkdirTemp(os.TempDir(), "dingofs-")
	if err != nil {
		return reconcile.Result{}, err
	}
	...
	conf := dfsSetting.Name + ".conf"
	confPath := filepath.Join(dfsSetting.ClientConfPath, conf)
...
func (m *SecretController) SetupWithManager(mgr ctrl.Manager) error {
	...
	return c.Watch(source.Kind(mgr.GetCache(), &corev1.Secret{}), &handler.EnqueueRequestForObject{}, predicate.Funcs{
